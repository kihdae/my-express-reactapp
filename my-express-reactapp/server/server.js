...
app.post('/api/journal', async (req, res) => {
  try {
    ...
    const userId = await User.findOne({ where: { name: req.body.content } });
    if (!userId) {
      return res.status(404).json({ error: 'endpoint f upped' });
    }
    // const user = await User.findByPk(userId, { include: JournalEntry });

    const newJournalEntry = await User.createJournalEntry({ content, userId });
